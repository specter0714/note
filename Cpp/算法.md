...
//加一点东西，dijkstra也可以记录最短路径的轨迹
//需要记录每个节点的前驱节点（就是说这个点最后一次是哪个点优化过来的）
//最后用DFS通过最后一个节点往前找可以输出最短路径轨迹
#include<iostream>
#include<algorithm>
#include<cstring>
#include<queue>
...
int main()
{
	...
	dfs(root);
	cout << max(ans[root][1], ans[root][0]);
	return 0;
}
```
