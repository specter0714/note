...
当主节点宕机时，主节点还没有同步从节点，然后Redis会选取一个从节点作为新的主节点，但是之前没有同步，导致锁失效

**解决方案**：将主从同步复制多个，每个主从同步群互不干扰，只有java应用发送的请求的响应都一致时才成功

![image-20250901125807807](../image/image-20250901125807807.png)

# Redis优化秒杀

* 新增秒杀优惠券的同时，将优惠券信息保存到Redis中
* 基于Lua脚本，判断秒杀库存，一人一单，决定用户是否抢购成功
* 如果抢购成功，将优惠卷id和用户信息封装后存入阻塞队列
* 开启线程任务，不断从阻塞队列中获取信息，实现异步下单功能

# 阻塞队列

**阻塞队列**，在普通队列的基础上增加了**阻塞机制**，当队列操作（入队/出队）无法立即执行时，会让线程进入等待（阻塞）状态，直到满足条件后自动唤醒。
...
brpop 和 lpush 是 redis 中用于操作列表（list）数据结构的命令。

### brpoop（阻塞式移除并获取列表最后一个元素）

### lpush（将一个或多个值插入到列表头部）

## 基于PubSub的消息队列

**PubSub**（发布订阅）是Redis2.0 版本引入的消息传递模型。消费者可以订阅一个或多个channel，生产者向对应channel发送消息后，所有订阅者都收到相关消息。
...
![image-20250905205821737](../image/image-20250905205821737.png)

### 发送消息xadd

![image-20250905205836442](../image/image-20250905205836442.png)

### 读取消息xread

![image-20250906134904585](../image/image-20250906134904585.png)

**注意：**当指定起始ID为$时，代表读取最新的消息，如果我们处理消息的过程中，又有超过 1 条以上的消息到达队列，则下次获取时也只能获取到最新的一条，会出现**漏读消息**的问题
...
* 消息标识：消费者组会维护一个标识，记录最后一个被处理的消息，哪怕消费者宕机重启，还会冲标识之后读取消息。确保每一个消息都会被消费
* 消息确认：消费者获取消息后，消息处于pending状态，并存入一个pending-list。当处理完成后需要通过XACK来确认消息，标记消息为已处理，才会从pending-list移除

#### 创建消费者组

```redis
xgroup create key groupName ID [mkstram]
```

* key：队列名称
* groupName：消费者组名称
* ID：起始ID标示，$表示队列中最后一个消息，0则代表队列中第一个消息
* mkstream：队列不存在时自动创建队列

#### 从消费者组中读取消息

![image-20250906143546176](../image/image-20250906143546176.png)

* group：消费者组名称

* consumer：消费者名称，如果消费者不存在，会自动创建一个消费者
* count：本次查询的最大数量
* block milliseconds：当没有消息时最长等待时间（阻塞时间），若不设置，没有消息直接返回空
* noack：用于改变消息的确认机制，让消费者获取消息后，无需执行 xack 命令确认，消息也不会进入 pending-list。
* streams key：指定队列名称
* ID：获取消息的起始ID：
* * “>”：从下一个未消费的消息开始
  * 其他：根据指定 id 从 pending-list 中获取已消费但未确认的消息，例如0，是从pending-list中的第一个消息开始

## 消息队列xreadgroup总结：

1. 消费者组获取消息后会自动把消息添加到pending-list
2. 必须手动xack，才能把消息从pending-list中移除
3. 获取消息，处理消息成功后，在执行xack
4. pending-list主要解决获取成功后服务宕机，但此时还未xack，就可以从pending-list中再次获取这个消息
...
**消费者相当于外部服务，多个进程的服务来处理读取的消息，因为redis操作是很快的，但是处理消息的速度更慢，就可以使用多个处理消息的消费者进程来处理从redis中读取的消息**

<img src="../image/image-20250906171541437.png" alt="image-20250906171541437" style="zoom:50%;" />

![image-20250906171643137](../image/image-20250906171643137.png)
...
* 只能排序：利用算法屏蔽违规的，用户不敢兴趣的内容，推送用户感兴趣的信息来吸引用户
* * 优点：用户粘度高，容易沉迷
  * 缺点：如果算法不够精准，可能起到反作用

## Feed流实现方案

* **拉模式：**也叫做读扩散，只有当用户打开收件箱的时候，才会从关注的人的发件箱根据时间拉取过来，内存占用低
* **推模式：**也叫写扩散，用户没有发件箱，他发布一个消息，会直接发送到关注他的人的收件箱，内存占用高
* **推拉结合模式：**也叫做读写混合模式，兼具推和拉两种模式的优点，活跃粉丝采用推模式，僵尸粉丝采用拉模式

## 基于推模式实现关注推送功能

* 推模式的实现：当user1发布blog时，从数据库查询关注user1的users的id，然后把blog的id储存在redis的ZSet数据结构中，**key**为$feed:users.getId()$，**value**为$blog.getId()$，**score**为$System.currentTimeMillis()$

* 用户分页查询关注的人发布的blog的实现

  ```java
  //1.分页查询参数
  // max：上一次查询的最小时间戳 | 当前时间
  // min：0
  // sffset：与上一次查询最小时间戳一直的所有元素的个数 | 0
  // count：分页大小
  ```

# GEO数据结构

GEO就是Geolocation的简写模式，代表地理坐标。Redis在3.2版本中加入了对GEO的支持，允许存储地理坐标信息，帮助我们根据经纬来检索数据。常见的命令有：

![image-20250910140358300](../image/image-20250910140358300.png)

# HyperLogLog用法

首先要搞懂两个概念：

* **UV：**全称Unique Visitor， 也叫独立访客量，是指通过互联网访问，浏览这个网页的自然人。一天内同一个用户多次访问该网站，只记录 1 次。
* **PV：**全称Page View，也叫页面访问量或点击量，用户每访问网站的一个页面，记录 1 次PV，用户多次打开页面，则记录多次PV。往往用来衡量网站的流量

UV统计在服务端做回比较麻烦，因为要判断该用户是否已经统计过了，需要将统计过的用户信息保存。但是如果每个访问的用户都保存到Redis中，数据量会非常恐怖。

redis中指令：pfadd(添加), pfcount(统计)
...
```java
super.remove(new LambdaQueryWrapper<Follow>()
                    .eq(Follow::getUserId, userId)
                    .eq(Follow::getFollowUserId, followUserId));
```

程序中使用stream流很方面，很简洁，很高级

```java
//4.根据id查询数据库，并封装成List<UserDTO>
        List<Long> ids = intersect.stream().map(Long::valueOf).toList();
        List<UserDTO> list = userService.listByIds(ids)
                .stream().map(op -> BeanUtil.copyProperties(op, UserDTO.class)).toList();
